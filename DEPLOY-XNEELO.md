# Putting Fungi Academy on centenarynetworks.com/fungiacademy

Fungi Academy is the same application as SPS Academy, in a second folder on the same
...
different company's learner records. We write to `public_html/fungiacademy/` and nowhere
...
```
cd ../sps
php tools/sync-backend.php --check ../fungi     # what has drifted
...
and without a merge. If Fungi needs something SPS does not, it becomes a key in
`lib/brand.php` and a `brand('key')` in the shared page — added in SPS, synced back out.

**What is genuinely Fungi's**, and is never touched by the sync:
...
| Secret | Value |
|---|---|
| `FUNGI_FTP_SERVER` | `centenarynetworks.com` |
| `FUNGI_FTP_USERNAME` | the Xneelo SFTP user |
| `FUNGI_FTP_PASSWORD` | its password |

The same Xneelo account as SPS today. If Xneelo's panel will give a second FTP user whose
home is `public_html/fungiacademy`, use that instead — then this repository's credential
...
Over SFTP, create `~/private/fungiacademy-config.php` — **outside** `public_html`, and
...
`public_html/fungiacademy/` looks for `fungiacademy-config.php` and will accept nothing
else. If it is missing the site refuses to start and says which file it wanted.

```php
'tenant'    => 'fungi',          // NOT 'sps'. This line is the entire separation.
...
`https://centenarynetworks.com/fungiacademy/setup`, run it — then **come back and empty
...
GitHub → Actions → **Deploy Fungi Academy** → Run workflow. Pushing does not deploy.
...
- `/contact` says "Fungi Academy" and does **not** carry SPS branding — this is the one
...
1. `https://centenarynetworks.com/fungiacademy/` — the Fungi homepage, teal, Fungi logo.
2. `/fungiacademy/contact` — send a real registration through it.
3. `/fungiacademy/admin` — sign in, and confirm that registration is the **only** thing
   listed. If SPS's registrations appear, stop: the configuration file is wrong.
4. `/fungiacademy/phpcheck.php` — should be a 404 once `setup_token` is empty.
...
- **The phone number in `lib/brand.php` is a placeholder** carried over from the static
  site. It needs Fungi's real switchboard before anyone is told the site is finished.
- **The company's registered name needs confirming.** The static site used
  "Fungi Utilities (Pty) Ltd" on one page and "Fungi — Digital Utility" on another.
...
- **The privacy notice is a draft** and names SPS's outstanding items too: each company's
  ...
  period. It should be read by Fungi before the site is announced.
